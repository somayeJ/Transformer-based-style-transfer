...
class StyleTransformer(nn.Module):
    def __init__(self, config, vocab):
        ...
        num_styles, num_layers = config.num_styles, config.num_layers # self.num_layers = 4
    ...
    def forward(self, inp_tokens, gold_tokens, inp_lengths, style,
        ...
        #print('max_enc_len, self.max_length',max_enc_len, self.max_length)
        #print('inp_tokens',inp_tokens)
...
class Decoder(nn.Module):
    def __init__(self, num_layers, d_model, vocab_size, h, dropout):
        ...
        self.layers = nn.ModuleList([DecoderLayer(d_model, h, dropout) for _ in range(num_layers)]) # decoder stacks
        self.norm = LayerNorm(d_model)
